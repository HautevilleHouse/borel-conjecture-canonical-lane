...
In-paper anchor: `paper/BOREL_CONJECTURE_PREPRINT.md` (`BOR_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving topological rigidity of aspherical manifolds through an admissible rigidity-transfer closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BOR_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and manifold windows do not collapse.
This is the first-failure capture contribution to `M_BOR`.
...
If near-failure extraction and no-Zeno spacing hold, then `BOR_G3` prevents hidden drift before rigidity is applied.
