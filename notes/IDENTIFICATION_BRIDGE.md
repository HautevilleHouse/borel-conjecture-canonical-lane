...
In-paper anchor: `paper/BOREL_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving topological rigidity of aspherical manifolds through an admissible rigidity-transfer closure architecture`.
...
- endpoint lock: `homeomorphism_transfer`.
...
2. the endpoint-transfer lock `homeomorphism_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_BOR` remains positive.
...
Payload: verify `homeomorphism_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_BOR` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `homeomorphism_transfer`
